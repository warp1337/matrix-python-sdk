...
class TestOlmDevice:
    ...
    def test_upload_identity_keys(self):
        ...
        self.device.one_time_key_counts = {}
        ...
        assert self.device.one_time_key_counts == resp['one_time_key_counts']
    ...
    def test_upload_identity_keys_invalid(self, proportion):
        with pytest.raises(ValueError):
            OlmDevice(self.cli.api,
                      self.user_id,
                      self.device_id,
                      signed_otk_proportion=proportion)
    ...
    def test_upload_one_time_keys(self, proportion):
        ...
        device = OlmDevice(
            self.cli.api, self.user_id, self.device_id, signed_otk_proportion=proportion)
        assert not device.one_time_key_counts
        ...
        assert device.one_time_key_counts == resp['one_time_key_counts']
    ...
    def test_upload_one_time_keys_force_update(self):
        upload_url = HOSTNAME + MATRIX_V2_API_PATH + '/keys/upload'
        self.device.one_time_key_counts = {'curve25519': 10}
        resp = deepcopy(example_key_upload_response)
        responses.add(responses.POST, upload_url, json=resp)
    ...
    def test_update_one_time_key_counts(self, count, should_upload):
        ...
        self.device.otk_target_counts['signed_curve25519'] = 50
        self.device.one_time_key_counts = {}
    ...
    def test_invalid_otk_threshold(self, threshold):
        with pytest.raises(ValueError):
            OlmDevice(self.cli.api,
                      self.user_id,
                      self.device_id,
                      otk_threshold=threshold)
