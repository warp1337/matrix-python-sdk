...
class TestCryptoStore(object):
    ...
    def test_megolm_inbound_persistence(self, curve_key, device):
        ...
        assert not sessions
        assert not self.store.get_inbound_session(session.id)
        ...
        saved_session = self.store.get_inbound_session(session.id)
        assert saved_session.id == session.id

        sessions = {}
        saved_session = self.store.get_inbound_session(session.id, sessions)
